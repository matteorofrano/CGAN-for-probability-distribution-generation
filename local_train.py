...
# --- Data volume ---
TOTAL_ROWS = 100_000
CHUNK_SIZE =    10_000          # paths per simulation batch (100 chunks of 100 K)
...
# --- Training ---
MAX_EPOCH        = 100
BATCH_SIZE       = 32             # ← CPU memory constraint: the LSTM processes
                                  #   BATCH_SIZE × N_SAMPLES_SR sequences in one
                                  #   forward call (= the effective LSTM batch).
                                  #   128 × 10 = 1280 sequences × 252 steps
                                  #   requires ~3 GB; 32 × 5 = 160 requires ~200 MB.
LR_G             = 1e-3
N_SAMPLES_SR     = 5              # ← keep product BATCH_SIZE × N_SAMPLES_SR ≤ ~200
                                  #   on CPU.  The paper shows m=3 already works well.
