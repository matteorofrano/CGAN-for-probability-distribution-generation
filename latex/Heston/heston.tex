...
Since equations (\ref{eq:logprice_sde_heston})--(\ref{eq:var_sde_heston}) do not admit closed-form solutions in general, so they must be integrated numerically. The proposed simulator supports two schemes:\\
...
The trained model is evaluated on a test set given by $100000$ trajectories that were not seen during training by using the same Heston parameters adopted for the training process and described in table \ref{tab:heston_training_params}. 
...
\toprule
Traj. & $X_T$ & $v_T$ & It\^{o} shift & $\mu_{\mathrm{th}}$
      & $\hat{\mu}_{\mathrm{true}}$ & $|\Delta_{\mathrm{true}}|$
      & $\hat{\mu}_{\mathrm{gen}}$\phantom{$|$} \\
\midrule
0 & $-1.219$ & $0.879$ & $-0.017$ & $-1.236$ & $-1.237$ & $0.001$ & $-1.243$ \\
1 & $-0.975$ & $1.094$ & $-0.021$ & $-0.996$ & $-0.983$ & $0.014$ & $-1.086$ \\
2 & $\phantom{-}0.721$ & $0.367$ & $-0.007$ & $\phantom{-}0.714$ & $\phantom{-}0.712$ & $0.001$ & $\phantom{-}0.675$ \\
3 & $-0.159$ & $0.498$ & $-0.010$ & $-0.169$ & $-0.174$ & $0.006$ & $-0.147$ \\
\bottomrule
\end{tabular}
\end{table}
...
This shift represents only $6$--$10\%$ of one standard deviation and is thus
barely detectable visually, explaining why the histogram peaks appear nearly
aligned with the $X_T$ marker in the plots.  The partial cancellation by the
positive mode-mean gap induced by the negative skewness ($\rho=-0.7$ implies
skewness $\approx -0.18$ to $-0.30$, shifting the mode $\approx +0.037$ above
the mean) further reduces the visible offset.\\
...
the theoretical and empirical cases.  The vol-of-vol contribution of $\sigma_v
= 0.9$ adds only $\approx 12.5\%$ to the standard deviation relative to the
naive $\sqrt{v_T\tau}$ floor, because the ten-step horizon $\tau$ is short
enough ($\kappa\tau = 0.079$) that the variance process has insufficient time
to deviate appreciably from $v_T$.  The generated standard deviations are also
