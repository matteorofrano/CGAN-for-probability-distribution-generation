...
\begin{algorithm}[H]
    \label{alg:frFFT}
\caption{Batched Fractional FFT via Bluestein's Identity}
\begin{algorithmic}[1]
\REQUIRE Input batch $\mathbf{X} \in \mathbb{C}^{B \times N}$, fractional parameter
         $\alpha = \eta\lambda / (2\pi)$
\ENSURE  Output batch $\mathbf{Y} \in \mathbb{C}^{B \times N}$ where
         $Y_{b,k} = \sum_{j=0}^{N-1} X_{b,j}\, e^{-i 2\pi\alpha j k}$

\LINECOMMENT{Compute next power-of-two FFT length to avoid circular aliasing}
\STATE $M \leftarrow 2^{\lceil \log_2(2N - 1) \rceil}$

\LINECOMMENT{Build the chirp sequence $\text{chirp}[j] = e^{i\pi\alpha j^2}$, $j = 0,\ldots,N-1$}
\STATE $\mathbf{j} \leftarrow [0,\, 1,\, \ldots,\, N-1]^{\top}$
\STATE $\mathbf{chirp} \leftarrow \exp\!\left(i\pi\alpha\,\mathbf{j}^{\odot 2}\right)
       \in \mathbb{C}^{N}$

\LINECOMMENT{Phase-modulate each input row by the conjugate chirp}
\FOR{$b = 1$ \textbf{to} $B$}
    \STATE $\mathbf{a}_{b} \leftarrow \mathbf{X}_{b} \odot \overline{\mathbf{chirp}}$
    \COMMENT{$a_{b,j} = X_{b,j}\, e^{-i\pi\alpha j^{2}}$}
\ENDFOR

\LINECOMMENT{Zero-pad modulated inputs to length $M$}
\STATE $\mathbf{a}_{\mathrm{pad}} \leftarrow \mathbf{0}_{B \times M}$
\STATE $\mathbf{a}_{\mathrm{pad}}[\,:,\; 0 : N] \leftarrow \mathbf{a}$

\LINECOMMENT{Build the circular convolution kernel of length $M$}
\STATE $\mathbf{h}_{\mathrm{pad}} \leftarrow \mathbf{0}_{M}$
\STATE $\mathbf{h}_{\mathrm{pad}}[0 : N]
       \leftarrow \mathbf{chirp}$
       \COMMENT{causal half: $h[0],\ldots, h[N-1]$}
\STATE $\mathbf{h}_{\mathrm{pad}}[M-N+1 : M]
       \leftarrow \mathrm{flip}\!\left(\mathbf{chirp}[1:]\right)$
       \COMMENT{acausal half: $h[-(N-1)],\ldots, h[-1]$}

\LINECOMMENT{Circular convolution via FFT (complexity $\mathcal{O}(M\log M)$)}
\STATE $\hat{\mathbf{A}} \leftarrow \mathrm{FFT}\!\left(\mathbf{a}_{\mathrm{pad}},\; \mathrm{axis}=1\right)$
       \COMMENT{shape $B \times M$}
\STATE $\hat{\mathbf{H}} \leftarrow \mathrm{FFT}\!\left(\mathbf{h}_{\mathrm{pad}}\right)$
       \COMMENT{shape $M$, shared across the batch}
\STATE $\mathbf{conv} \leftarrow
       \mathrm{IFFT}\!\left(\hat{\mathbf{A}} \odot \hat{\mathbf{H}},\; \mathrm{axis}=1\right)$
       \COMMENT{shape $B \times M$}

\LINECOMMENT{Multiply first $N$ outputs by the conjugate chirp to complete Bluestein's identity}
\STATE $\mathbf{Y} \leftarrow
       \mathbf{conv}[\,:,\; 0 : N]
       \odot \overline{\mathbf{chirp}}$
       \COMMENT{$Y_{b,k} = e^{-i\pi\alpha k^{2}}\,\mathrm{conv}_{b,k}$}
