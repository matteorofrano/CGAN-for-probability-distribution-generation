...
A set of $J = 1000$ independent trajectories is simulated using the
\texttt{DataSimulator} class described in \ref{alg:y condition}, with $N = 50$ time steps over a horizon of
...
and a frequency-domain step $\eta = 0.25$.  Following algorithm \ref{alg:frFFT}. The fractional parameter is set to
...
using the batched fractional FFT described in algorithm \ref{alg:frFFT},
...
The code simulates \texttt{ 10,000,000} trajectories, each of length \texttt{22 + 1}. To avoid memory overflow, the trajectories are generated in chunks of size $100,000$.
...
\begin{itemize}
    \item the record length, equal to \texttt{N\_STEPS = 22 + 1},
...
\begin{itemize}
    \item latent noise dimension \texttt{Z\_NOISE\_DIM = 32},
