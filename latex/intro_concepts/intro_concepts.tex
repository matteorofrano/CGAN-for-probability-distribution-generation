...
The prediction of financial processes is
...
For this reason, probabilistic forecasting has attracted increasing attention in both financial econometrics and machine learning.
Rather than predicting a single value, 
probabilistic models estimate the entire conditional distribution of future prices given the available historical information.
...
The rapid development of deep learning has introduced a new family of probabilistic models capable of approximating highly complex data distributions.
Among these, Generative Adversarial Networks (GANs) have emerged as one of the most successful generative modeling techniques.
...
Recent research has extended GANs to time-series forecasting through conditional architectures,
where the generator produces future observations conditioned on historical data.\\

The objective of this thesis is to investigate the use of generative models for probabilistic forecasting of financial processes and, 
...
In quantitative risk management, the predicted distribution of future asset prices can be directly employed to estimate risk measures such as Value-at-Risk (VaR) and Expected Shortfall (ES), allowing financial institutions to quantify the probability and magnitude of adverse market movements without relying on restrictive distributional assumptions.
Similarly, probabilistic forecasts provide a natural framework for scenario generation and stress testing, enabling the simulation of realistic future market conditions for regulatory compliance and internal capital assessment.
Another important application concerns derivative pricing, in fact, under the risk-neutral valuation framework, 
...
Unlike traditional approaches that require the explicit specification and calibration of a stochastic model, a data-driven probabilistic forecasting model has the potential to learn complex conditional distributions directly from historical observations, 
providing a flexible alternative when the underlying market dynamics deviate from standard analytical assumptions.\\

The work begins with Conditional GAN architectures that directly generate future probability distributions conditioned on historical asset trajectories.
...
Initially, experiments are conducted assuming assets' dynamics can be modelled by Black and Scholes model, where analytical probability distributions are available and provide an exact benchmark for model evaluation.
...
The remainder of this thesis is organized as follows: chapter 2 introduces the Conditional GAN and Conditional Wasserstein GAN architectures and presents the corresponding experimental results; 
